...
describe('Android release configuration', () => {
  it('uses the expected app version and Android build code for the release', () => {
    ...
    expect(source).toContain('versionCode: 26');
    expect(JSON.parse(readFileSync(packageJsonPath, 'utf8')).version).toBe('1.0.10');
    expect(JSON.parse(readFileSync(serverPackageJsonPath, 'utf8')).version).toBe('1.0.10');
  });
