    ...
    <View style={styles.root}>
      ...
      <View style={styles.container}>
        <View style={styles.headerRow}>
          ...
          <View><Text style={styles.title}>{isSupported ? '📡 غرف Wi‑Fi المحلية' : '🌐 غرف الويب برمز'}</Text><Text style={styles.subtitle}>{isSupported ? 'mDNS + TCP مباشر — دون إنترنت أو خادم خارجي' : 'WebSocket آمن — أنشئ رمزاً وشاركه مع صديقك'}</Text></View>
        </View>
        ...
        <Text style={styles.footer}>{Platform.OS === 'web' ? 'تحتاج غرف الويب إلى رابط WebSocket آمن مهيأ عند النشر.' : 'يشترط عدم تفعيل AP Isolation في الراوتر'}</Text>
      </View>
    </View>
  );
}
