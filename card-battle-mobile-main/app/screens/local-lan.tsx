...
export default function LocalLanScreen() {
  ...
  const { rooms, hostedRoom, state, notice, peer, lastGameEvent, isSupported, hostRoom, refreshRooms, joinRoom, sendGameEvent, leave } = useLanMultiplayer();
  ...
  useEffect(() => {
    ...
    return () => leave();
  }, [isSupported, leave, refreshRooms]);
          ...
          <View style={styles.leftPane}>
            ...
            {!isSupported ? (
              ...
              <>
                ...
                {state === 'connected' && <TouchableOpacity style={styles.testButton} onPress={() => sendGameEvent('LAN_READY', { sentAt: Date.now() })}><Text style={styles.testText}>إرسال رسالة لعب مباشرة</Text></TouchableOpacity>}
                {!!lastGameEvent && <Text style={styles.eventNote}>وصل إجراء محلي: {lastGameEvent.event}</Text>}
              </>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  infoBox: { padding: SPACE.md, borderRadius: RADIUS.md, backgroundColor: 'rgba(92,58,24,0.55)', borderWidth: 1, borderColor: 'rgba(245,158,11,0.55)' }, infoTitle: { color: '#f6c76f', fontWeight: '900', textAlign: 'right' }, infoText: { color: '#e4c798', marginTop: SPACE.xs, fontSize: FONT.xs, textAlign: 'right', lineHeight: 19 }, testButton: { minHeight: 45, borderRadius: RADIUS.sm, backgroundColor: '#4f46a5', justifyContent: 'center', alignItems: 'center', marginTop: SPACE.sm }, testText: { color: '#fff', fontWeight: '900', fontSize: FONT.sm }, eventNote: { color: '#86efac', fontSize: FONT.xs, textAlign: 'right' }, footer: { color: 'rgba(200,190,210,0.65)', fontSize: 10, textAlign: 'center', marginTop: SPACE.xs },
});
