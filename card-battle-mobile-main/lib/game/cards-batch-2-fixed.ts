import { Card, Element, ElementAdvantage, Effect, ELEMENT_ADVANTAGES, ELEMENT_WEAKNESSES, ELEMENT_MULTIPLIER } from './types';

// Batch 2/6 — Dragon Ball, AoT, Death Note, Demon Slayer + Unique cards from Batch 1
export const CARDS_BATCH_2: Card[] = [
    {
        id: 'yamcha',
        name: 'Yamcha',
        nameAr: 'يامتشا',
        attack: 6, defense: 6,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'rare', stars: 3,
        animationPreset: 'fire',
    },
    {
        id: 'tien_shinhan',
        name: 'Tien Shinhan',
        nameAr: 'تيان شينهان',
        attack: 8, defense: 9,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'rare', stars: 3,
        animationPreset: 'fire',
    },
    {
        id: 'krillin',
        name: 'Krillin',
        nameAr: 'كريلين',
        attack: 15, defense: 13,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'rare', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'master_roshi',
        name: 'Master Roshi',
        nameAr: 'الماستر روشي',
        attack: 12, defense: 11,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'rare', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'piccolo',
        name: 'Piccolo',
        nameAr: 'بيكولو',
        attack: 18, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'Epic', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'trunks',
        name: 'Trunks',
        nameAr: 'ترانكس',
        attack: 17, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'gohan',
        name: 'Gohan',
        nameAr: 'غوهان',
        attack: 16, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'cell',
        name: 'Cell',
        nameAr: 'سيل',
        attack: 17, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'frieza',
        name: 'Frieza',
        nameAr: 'فريزا',
        attack: 17, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'vegeta',
        name: 'Vegeta',
        nameAr: 'فيجيتا',
        attack: 17, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'broly',
        name: 'Broly',
        nameAr: 'بروللي',
        attack: 18, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'son_goku',
        name: 'Son Goku',
        nameAr: 'غوكو',
        attack: 18, defense: 17,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'hitch_dreyse',
        name: 'Hitch Dreyse',
        nameAr: 'هيتش دريس',
        attack: 1, defense: 1,
        gender: 'female',
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'common', stars: 1,
        animationPreset: 'default',
    },
    {
        id: 'sasha_braus',
        name: 'Sasha Braus',
        nameAr: 'ساشا براوس',
        attack: 8, defense: 6,
        gender: 'female',
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'rare', stars: 3,
        animationPreset: 'default',
    },
    {
        id: 'falco_grice',
        name: 'Falco Grice',
        nameAr: 'فالكو غرايس',
        attack: 9, defense: 7,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'rare', stars: 3,
        animationPreset: 'default',
    },
    {
        id: 'armin_arlert',
        name: 'Armin Arlert',
        nameAr: 'أرمين أرليرت',
        attack: 13, defense: 11,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'bertholdt_hoover',
        name: 'Bertholdt Hoover',
        nameAr: 'بيرتولدت هوفر',
        attack: 12, defense: 12,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'porco_galliard',
        name: 'Porco Galliard',
        nameAr: 'بوركو غاليارد',
        attack: 14, defense: 11,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'annie_leonhart',
        name: 'Annie Leonhart',
        nameAr: 'آني ليونهارت',
        attack: 14, defense: 11,
        gender: 'female',
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'mikasa_ackerman',
        name: 'Mikasa Ackerman',
        nameAr: 'ميكاسا أكرمان',
        attack: 15, defense: 14,
        gender: 'female',
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'reiner_braun',
        name: 'Reiner Braun',
        nameAr: 'راينر براون',
        attack: 16, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'zeke_yeager',
        name: 'Zeke Yeager',
        nameAr: 'زيك ييغر',
        attack: 16, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'eren_yeager',
        name: 'Eren Yeager',
        nameAr: 'إيرين ييغر',
        attack: 16, defense: 17,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'levi_ackerman',
        name: 'Levi Ackerman',
        nameAr: 'ليفي أكرمان',
        attack: 18, defense: 16,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'misa_amane',
        name: 'Misa Amane',
        nameAr: 'ميسا أماني',
        attack: 1, defense: 2,
        gender: 'female',
        race: 'human', cardClass: 'swordsman', element: 'water',
        tags: ["sword"],
        rarity: 'common', stars: 1,
        animationPreset: 'default',
        rageMode: {
            enabled: true,
            rageNameAr: 'عيون الشينيغامي',
            rageAttackBoost: 10,
            rageDefenseBoost: 8,
            oncePer: 'match',
        },
    },
    {
        id: 'inosuke_hashibira',
        name: 'Inosuke Hashibira',
        nameAr: 'إينوسوكي هاشيبيرا',
        attack: 13, defense: 11,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'zenitsu_agatsuma',
        name: 'Zenitsu Agatsuma',
        nameAr: 'زينيتسو أغاتسوما',
        attack: 13, defense: 11,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'nezuko_kamado',
        name: 'Nezuko Kamado',
        nameAr: 'نيزوكو كامادو',
        attack: 13, defense: 11,
        gender: 'female',
        race: 'demon', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'fire',
        cardEffects: ['poison'],
    },
    {
        id: 'shinobu_kocho',
        name: 'Shinobu Kocho',
        nameAr: 'شينوبو كوتشو',
        attack: 12, defense: 11,
        gender: 'female',
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'tanjiro_kamado',
        name: 'Tanjiro Kamado',
        nameAr: 'تانجيرو كامادو',
        attack: 15, defense: 12,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'mitsuri_kanroji',
        name: 'Mitsuri Kanroji',
        nameAr: 'ميتسوري كانروجي',
        attack: 13, defense: 14,
        gender: 'female',
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'gyutaro',
        name: 'Gyutaro',
        nameAr: 'غيوتارو',
        attack: 14, defense: 13,
        race: 'demon', cardClass: 'swordsman', element: 'fire',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'doma',
        name: 'Doma',
        nameAr: 'دوما',
        attack: 16, defense: 14,
        race: 'demon', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'akaza',
        name: 'Akaza',
        nameAr: 'أكازا',
        attack: 16, defense: 14,
        race: 'demon', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'kokushibo',
        name: 'Kokushibo',
        nameAr: 'كوكوشيبو',
        attack: 17, defense: 16,
        race: 'demon', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'muzan_kibutsuji',
        name: 'Muzan Kibutsuji',
        nameAr: 'موزان كيبوتسوجي',
        attack: 18, defense: 17,
        race: 'demon', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'yoriichi_tsugikuni',
        name: 'Yoriichi Tsugikuni',
        nameAr: 'يوريتشي تسوغيكوني',
        attack: 18, defense: 17,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    // ─── كروت منقولة من Batch 1 (فريدة — غير مكررة في أي batch آخر) ────────
    {
        id: 'Turin',
        name: 'Turin',
        nameAr: 'تورين',
        attack: 12, defense: 10,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'jinbe',
        name: 'Jinbe',
        nameAr: 'جينبي',
        attack: 15, defense: 14,
        race: 'orc', cardClass: 'swordsman', element: 'water',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'marco',
        name: 'Marco',
        nameAr: 'ماركو',
        attack: 16, defense: 15,
        race: 'orc', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'law',
        name: 'Law',
        nameAr: 'لاو',
        attack: 16, defense: 15,
        race: 'human', cardClass: 'swordsman', element: 'water',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'katakuri',
        name: 'Katakuri',
        nameAr: 'كاتاكوري',
        attack: 17, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'dracule_mihawk',
        name: 'Dracule Mihawk',
        nameAr: 'دراكول ميهوك',
        attack: 19, defense: 17,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'roronoa_zoro',
        name: 'Roronoa Zoro',
        nameAr: 'رورونوا زورو',
        attack: 18, defense: 16,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'monkey_d_luffy',
        name: 'Monkey D. Luffy',
        nameAr: 'مونكي دي لوفي',
        attack: 18, defense: 17,
        race: 'human', cardClass: 'swordsman', element: 'lightning',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'lightning',
    },
    {
        id: 'sakura_haruno',
        name: 'Sakura Haruno',
        nameAr: 'ساكورا هارونو',
        attack: 3, defense: 4,
        gender: 'female',
        race: 'human', cardClass: 'healer', element: 'earth',
        tags: ["magic"],
        rarity: 'common', stars: 1,
        animationPreset: 'default',
    },
    {
        id: 'nagato',
        name: 'Nagato',
        nameAr: 'ناغاتو',
        attack: 17, defense: 16,
        race: 'human', cardClass: 'swordsman', element: 'fire',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'fire',
    },
    {
        id: 'villager_a',
        name: 'Villager A',
        nameAr: 'مواطن عادي',
        attack: 1, defense: 1,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'common', stars: 1,
        animationPreset: 'default',
    },
    {
        id: 'villager_b',
        name: 'Villager B',
        nameAr: 'مواطن ب',
        attack: 1, defense: 1,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'common', stars: 1,
        animationPreset: 'default',
    },
    {
        id: 'villager_c',
        name: 'Villager C',
        nameAr: 'مواطن ج',
        attack: 1, defense: 1,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'common', stars: 1,
        animationPreset: 'default',
    },
    {
        id: 'mob',
        name: 'Mob',
        nameAr: 'موب',
        attack: 1, defense: 1,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'common', stars: 1,
        animationPreset: 'default',
    },
    {
        id: 'guts',
        name: 'Guts',
        nameAr: 'غاتس',
        attack: 18, defense: 16,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'griffith',
        name: 'Griffith',
        nameAr: 'غريفيث',
        attack: 17, defense: 17,
        race: 'human', cardClass: 'swordsman', element: 'wind',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'elden_ring_knight',
        name: 'Elden Ring Knight',
        nameAr: 'فارس إلدن رينغ',
        attack: 10, defense: 13,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'ranni',
        name: 'Ranni',
        nameAr: 'راني',
        attack: 13, defense: 11,
        gender: 'female',
        race: 'elf', cardClass: 'mage', element: 'water',
        tags: ["magic"],
        rarity: 'epic', stars: 4,
        animationPreset: 'default',
    },
    {
        id: 'melina',
        name: 'Melina',
        nameAr: 'ميلينا',
        attack: 10, defense: 9,
        gender: 'female',
        race: 'elf', cardClass: 'healer', element: 'fire',
        tags: ["magic"],
        rarity: 'epic', stars: 4,
        animationPreset: 'fire',
    },
    {
        id: 'malenia',
        name: 'Malenia',
        nameAr: 'مالينيا',
        attack: 18, defense: 16,
        gender: 'female',
        race: 'elf', cardClass: 'swordsman', element: 'wind',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
        cardEffects: ['charge'],
    },
    {
        id: 'radahn',
        name: 'Radahn',
        nameAr: 'رادان',
        attack: 18, defense: 17,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'morgott',
        name: 'Morgott',
        nameAr: 'مورغوت',
        attack: 17, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'maliketh',
        name: 'Maliketh',
        nameAr: 'ماليكيث',
        attack: 18, defense: 16,
        race: 'orc', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
    {
        id: 'tarnished',
        name: 'Tarnished',
        nameAr: 'التارنيشد',
        attack: 17, defense: 16,
        race: 'human', cardClass: 'swordsman', element: 'earth',
        tags: ["sword", "crown"],
        rarity: 'legendary', stars: 5,
        animationPreset: 'default',
    },
];
