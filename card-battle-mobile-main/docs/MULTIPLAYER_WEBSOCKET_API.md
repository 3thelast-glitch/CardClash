...
ينشئ لاعب مضيف وغرفة جديدة. عند عدم إرسال رمز دعوة، يولد الخادم رمزاً من ستة أحرف/أرقام. ويمكن للمضيف اختيار رمز خاص من 4 إلى 8 أحرف أو أرقام لمشاركته مع الأصدقاء.
...
| `inviteCode` | لا | رمز دعوة مخصص من 4 إلى 8 أحرف أو أرقام. يحوله الخادم إلى أحرف كبيرة ويرفض الرموز غير الصالحة أو المستخدمة مسبقاً. |

**الردود المحتملة:** `ROOM_CREATED` للمضيف، أو `ERROR` إذا كان رمز الدعوة غير صالح أو مستخدماً.

> تنشئ واجهة التطبيق رابط دعوة عميقاً يحتوي على الرمز؛ عند فتحه يملأ رمز الغرفة تلقائياً في شاشة الانضمام. يبقى إدخال الرمز يدوياً متاحاً كخيار احتياطي.
